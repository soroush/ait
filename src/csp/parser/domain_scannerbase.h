...
#ifndef DomainLexerBASE_H_INCLUDED
#define DomainLexerBASE_H_INCLUDED
...
public:
    DomainLexerBase(DomainLexerBase const &other)             = delete;
    DomainLexerBase &operator=(DomainLexerBase const &rhs)    = delete;
...
protected:
    DomainLexerBase(std::istream &in, std::ostream &out);
    DomainLexerBase(std::string const &infilename, std::string const &outfilename);
...
inline std::ostream &DomainLexerBase::out()
{
    return *d_out;
}

inline void DomainLexerBase::push(size_t ch)
{
    d_input.reRead(ch);
}

inline void DomainLexerBase::push(std::string const &str)
{
    d_input.reRead(str, 0);
}

inline bool DomainLexerBase::atFinalState()
{
    return d_final.notAtBOL.rule != std::numeric_limits<size_t>::max() || 
            (d_atBOL && d_final.atBOL.rule != std::numeric_limits<size_t>::max());
}

inline void DomainLexerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void DomainLexerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &DomainLexerBase::matched() const
{
    return d_matched;
}

inline DomainLexerBase::StartCondition__ DomainLexerBase::startCondition() const
{
    return d_startCondition;
}

inline std::string const &DomainLexerBase::filename() const
{
    return d_filename;
}

inline void DomainLexerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t DomainLexerBase::length() const
{
    return d_matched.size();
}

inline void DomainLexerBase::leave(int retValue) const
{
    throw static_cast<Leave__>(retValue);
}

inline size_t DomainLexerBase::lineNr() const
{
    return d_input.lineNr();
}

inline void DomainLexerBase::more()
{
    d_more = true;
}

inline void DomainLexerBase::begin(StartCondition__ startCondition)
{
    d_dfaBase__ = 
        s_dfaBase__[static_cast<int>(d_startCondition = startCondition)];
}

inline size_t DomainLexerBase::state__() const
{
    return d_state;
}

inline size_t DomainLexerBase::get__()
{
    return d_input.get();
}

inline bool DomainLexerBase::return__()
{
    return d_return;
}

inline void DomainLexerBase::noReturn__()
{
    d_return = false;
}


#endif //  DomainLexerBASE_H_INCLUDED
